    ...
    for i in range(segments + 1):
        ...
        for j in [-width / 2, width / 2]:
            ...
            if not color:
                vcols.append(
                    (
                        int(127 + 128 * math.cos(t)),
                        int(127 + 128 * math.sin(t)),
                        int(255 * (j + width / 2) / width),
                    )
                )
...
def build_klein_bottle(segments_u=50, segments_v=20, scale=1.0, color=None):
    ...
    for i in range(segments_u + 1):
        ...
        for j in range(segments_v + 1):
            ...
            if u < math.pi:
                x = 3 * math.cos(u) * (1 + math.sin(u)) + scale * (
                    2 * (1 - math.cos(u) / 2)
                ) * math.cos(v)
                ...
                y = 8 * math.sin(u)
            z = (
                scale * math.sin(u) + 2 * (1 - math.cos(u) / 2) * math.sin(v)
                if u < math.pi
                else scale * math.sin(v)
            )
...
def build_heart(scale=1.0, segments=50, color=None):
    ...
    for i in range(segments + 1):
        ...
        for j in range(segments + 1):
            ...
            if not color:
                vcols.append(
                    (255, int(100 + 155 * i / segments), int(100 + 155 * j / segments))
                )
...
def build_conical_helix(radius=1.0, height=3.0, turns=5, segments=100, color=None):
    ...
    if color:
        ...
        all_vcols = vcols + [
            (255, int(255 * i / segments), 0) for i in range(segments + 1)
        ]
...
def build_catenoid(
    ...
    for i in range(segments_u + 1):
        ...
        for j in range(segments_v + 1):
            ...
            if not color:
                vcols.append(
                    (int(127 + 128 * math.cos(v)), int(127 + 128 * math.sin(v)), 255)
                )
...
def build_capsule(radius=0.5, height=2.0, segments=30, color=None):
    ...
    for i in range(num_lat - 1):
        for j in range(num_verts_per_ring):
            ...
            i1 = (
                ring_start_idx_bottom
                + i * num_verts_per_ring
                + ((j + 1) % num_verts_per_ring)
            )
            i2 = ring_start_idx_bottom + (i + 1) * num_verts_per_ring + j
            i3 = (
                ring_start_idx_bottom
                + (i + 1) * num_verts_per_ring
                + ((j + 1) % num_verts_per_ring)
            )
            faces.append((i0, i1, i3))
            faces.append((i0, i3, i2))
