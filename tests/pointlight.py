...
def main():
    ...
    light = PointLight(position=Vec3(0.0, -3.5, 0.0), color=(255, 255, 255), intensity=3.0)
    engine.add_light_node(light) # Use add_light_node instead of the deprecated add_pointlight
    ...
    sphere_left_mesh = build_uv_sphere(radius=1.0, segments_x=32, segments_y=16, color=(200, 100, 100))
    # Add the sphere as a node and set its position
    sphere_left_node = engine.add_mesh_node(sphere_left_mesh) # Use add_mesh_node
    engine.set_node_position(sphere_left_node, -2.5, 0, 0) # Use set_node_position on the node

    # Create the second UV sphere and position it to the right
    sphere_right_mesh = build_uv_sphere(radius=1.0, segments_x=32, segments_y=16, color=(100, 100, 200))
    # Add the sphere as a node and set its position
    sphere_right_node = engine.add_mesh_node(sphere_right_mesh) # Use add_mesh_node
    engine.set_node_position(sphere_right_node, 2.5, 0, 0) # Use set_node_position on the node
