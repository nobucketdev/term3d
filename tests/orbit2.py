...
import math
import time
from term3d.core import term3d, Vec3, DirectionalLight, SceneNode
from term3d.shpbuild import build_uv_sphere

def main():
    """
    Main function to set up and run the example scene.
    """
    try:
        # Create the 3D engine instance
        engine = term3d(120, 40)
        engine.set_title("Parent-Child-Grandchild Orbit Example")
        ...
        # Set the child sphere's local position relative to the parent
        ...
        # The grandchild is added as a child of the child_node
        grandchild_node = child_node.add(SceneNode("Grandchild Sphere"))
        grandchild_node.mesh = grandchild_mesh
        # Set the grandchild's local position relative to the child
        # The grandchild's position will be (0, 0, 2.5) relative to the child's center
